...
import (
	...
	"github.com/arieffian/watchtower/internal/actions/mocks"
	"github.com/arieffian/watchtower/pkg/registry/auth"

	wtTypes "github.com/arieffian/watchtower/pkg/types"
...
var _ = Describe("the auth module", func() {
	...
	Describe("GetAuthURL", func() {
		It("should create a valid auth url object based on the challenge header supplied", func() {
			...
			imageRef, err := ref.ParseNormalizedNamed("arieffian/watchtower")
			Expect(err).NotTo(HaveOccurred())
			expected := &url.URL{
				Host:     "ghcr.io",
				Scheme:   "https",
				Path:     "/token",
				RawQuery: "scope=repository%3Aarieffian%2Fwatchtower%3Apull&service=ghcr.io",
			}
		...
		When("deriving the auth scope from an image name", func() {
			...
			It("should not include vanity hosts\"", func() {
				Expect(getScopeFromImageAuthURL("docker.io/arieffian/watchtower")).To(Equal("arieffian/watchtower"))
				Expect(getScopeFromImageAuthURL("index.docker.io/arieffian/watchtower")).To(Equal("arieffian/watchtower"))
			})
			It("should not destroy three segment image names\"", func() {
				Expect(getScopeFromImageAuthURL("piksel/arieffian/watchtower")).To(Equal("piksel/arieffian/watchtower"))
				Expect(getScopeFromImageAuthURL("ghcr.io/piksel/arieffian/watchtower")).To(Equal("piksel/arieffian/watchtower"))
			})
			It("should not prepend library/ to image names if they're not on dockerhub", func() {
				...
				Expect(getScopeFromImageAuthURL("ghcr.io/arieffian/watchtower")).To(Equal("arieffian/watchtower"))
			})
		})
	...
	Describe("GetChallengeURL", func() {
		It("should create a valid challenge url object based on the image ref supplied", func() {
			...
			imageRef, _ := ref.ParseNormalizedNamed("ghcr.io/arieffian/watchtower:latest")
		...
		It("should assume Docker Hub for image refs with no explicit registry", func() {
			...
			imageRef, _ := ref.ParseNormalizedNamed("arieffian/watchtower:latest")
			Expect(auth.GetChallengeURL(imageRef)).To(Equal(expected))
		})
		It("should use index.docker.io if the image ref specifies docker.io", func() {
			expected := url.URL{Host: "index.docker.io", Scheme: "https", Path: "/v2/"}
			imageRef, _ := ref.ParseNormalizedNamed("docker.io/arieffian/watchtower:latest")
			Expect(auth.GetChallengeURL(imageRef)).To(Equal(expected))
		})
	})
})
