...
```bash
docker run [...] -v <PATH>/config.json:/config.json arieffian/watchtower
```
...
```bash
docker run [...] -v <PATH_TO_HOME_DIR>/.docker/config.json:/config.json arieffian/watchtower
```
...
services:
  watchtower:
    image: arieffian/watchtower:latest
    volumes:
