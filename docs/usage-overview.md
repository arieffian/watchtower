Watchtower is itself packaged as a Docker container so installation is as simple as pulling the `arieffian/watchtower` image. If you are using ARM based architecture, pull the appropriate `arieffian/watchtower:armhf-<tag>` image from the [containrrr Docker Hub](https://hub.docker.com/r/arieffian/watchtower/tags/).
...
docker run -d \
  ...
  -v /var/run/docker.sock:/var/run/docker.sock \
  arieffian/watchtower container_to_watch --debug
```
